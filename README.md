...
`127.0.0.1:8080:8080` и не запускает ACME. Внутри контейнера процесс всё ещё
слушает `0.0.0.0`, чтобы работали Compose-сети и reverse proxy. Для локального
TLS + Basic Auth используйте профиль `auth-tls` и secret file:
