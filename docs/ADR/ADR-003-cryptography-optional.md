...
- AES-256-GCM encryption for OPSEC uploads (superior to XOR+HMAC baseline).
...
- Falls back to stdlib primitives for OPSEC (XOR+HMAC) and disables the
...
- `pip install exphttp` still works on any Python 3.10+; the minimal path
  serves files and runs OPSEC-in-XOR-mode.
...
- Two code paths to maintain for OPSEC encryption (baseline + AEAD).
